...
//input $mang=$product->values output: array('size'=>array(S,M),'color'=>array('Do',Xanh))
...
{
    foreach ($product->variant as $row) { // chuyen object->mang
        $mang=array();
        foreach ($row->values as $value) {
            $mang[]=$value->id;
        }
        if (array_diff($mang,$array)==NULL) { //so sanh 2 mang
            return false;
        }
    }
    return true;
