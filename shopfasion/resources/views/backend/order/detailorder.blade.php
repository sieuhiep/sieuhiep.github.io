					...
					<div class="panel-body">
						<div class="bootstrap-table">
							<div class="table-responsive">
									...
									<tbody>
										...
										<td>{{$item->id}}</td>
												<td>
													<div class="row">
														<div class="col-md-4">
															<img width="100px" src="public/backend/img/{{ $item->img }}" class="thumbnail">
														</div>
														<div class="col-md-8">
															<p>Tên Sản phẩm: <strong>{{ $item->name }}</strong></p>
																@foreach ($item->attr as $attr)
																<div class="group-color"><p>{{ $attr->name }}:{{ $attr->value }}</p>
																	</div>
																@endforeach
														</div>
													</div>
								...
								<table class="table">
									<thead>
										<tr>
											...
											<th>
											<h4 align='right' style="color: brown;">{{$customer->total}}</h4>
											</th>
								...
								<a href="admin/order/active/{{$customer->id}}" name="" id="" class="btn btn-success" role="button">Đã xử lý</a>
								</div>
							</div>
						</div>
