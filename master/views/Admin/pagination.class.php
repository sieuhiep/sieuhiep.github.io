...
class PerPage {
	...
	function getPrevNext($count,$href) {
		...
		if($pages>1) {
			if($_GET["page"] == 1) 
				$output = $output . '<span class="link disabled first">Trước</span>';
			else	
				$output = $output . '<a class="link first" onclick="getresult(\'' . $href . ($_GET["page"]-1) . '\')" >Trước</a>';			
			
			if($_GET["page"] < $pages)
				$output = $output . '<a  class="link" onclick="getresult(\'' . $href . ($_GET["page"]+1) . '\')" >Sau</a>';
			else				
				$output = $output . '<span class="link disabled">Sau</span>';
